...
def compare_templates(tree: Tree, templates: list) -> bool:
    """Check if one of templates fits given tree."""
...
def compare_step(tree: Tree, template: Tree) -> bool:
    """Check if the template fits the tree."""
    if template == '*':
        return True
...
def get_node_by_label(tree: Tree, template: Tree):
    """Return list of nodes, that is good for next layer."""
    res_list = []
