...
def extract_words(tree: Tree, template: Tree) -> str:
    """Return string of founded subtree."""
    list_words = find_step(tree, template)
    return " ".join(list_words)


def find_step(tree: Tree, template: Tree) -> list:
    """Return list of words from target subtree."""
    if template == "@":
        if type(tree) == str:
            return [tree]
        return tree.leaves()
...
def find_star_step(tree: Tree, template: Tree) -> list:
    """Next step for finding with * parameter."""
